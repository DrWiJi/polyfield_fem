...
class BoundaryConditionsViewport(MainViewport):
    ...
    def _refresh_boundary_conditions(self) -> None:
        ...
        if not self._plotter or not pv:
            return
        # Remove old BC actors (prefixed with bc_)
        for name in list(self._extra_actors.keys()):
    ...
    def _create_bc_primitive(self, bc) -> Any:
        ...
        if bc_type == "sphere":
            ...
            mesh = pv.Plane(center=(0, 0, 0), direction=(0, 0, 1), i_size=2, j_size=2)
        else:
...
class TopologyViewport(QWidget):
    """
    Viewport только для отображения сгенерированной 3D топологии.
    Меши не отображаются. Топология показывается в виде параллелепипедов (боксов).
    """
    ...
    def set_topology(self, topology: dict | None) -> None:
        """
        Устанавливает топологию для отображения.
        topology: dict с element_position_xyz [n,3], element_size_xyz [n,3]
        """
        if getattr(self, "_viewport_closed", False):
            return
        if not self._plotter or not pv:
        ...
        try:
            ...
            n = pos_arr.shape[0]
            # Строим один UnstructuredGrid из всех боксов (параллелепипедов)
            # Каждый бокс: 8 вершин, 1 ячейка типа 12 (VTK_HEXAHEDRON)
            points_list = []
            cells_list = []
            voff = 0
            for i in range(n):
            ...
            cell_types_arr = np.full(n, 12, dtype=np.uint8)  # VTK_HEXAHEDRON
